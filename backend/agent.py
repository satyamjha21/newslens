...
Example: ['query one', 'query two']"""
    ...
    try:
        ...
        logger.warning("Agent generate_keywords error: %s", exc)
        return FALLBACK_KEYWORDS
...
async def search_keyword(keyword: str) -> list[dict]:
    ...
    try:
        ...
        logger.warning("Agent search_keyword error for '%s': %s", keyword, exc)
        return []
...
async def run_agent():
    ...
    for keyword in keywords:
        ...
        for r in results:
            ...
            if database.url_exists(url):
                ...
                continue
            try:
                classification = classifier.classify_with_cache(
                    {"url": url, "title": title, "source": _extract_domain(url)},
                    lambda t, s: classifier.normalize_result(
                        dict(classifier.DEFAULT_RESULT), t
                    ),
                )
            except Exception as exc:
                logger.warning("Agent classify error: %s", exc)
                continue

            if database.insert_article(
                {
                    "title": title,
                    "url": url,
    ...
    logger.info("Agent cycle done — saved %s, skipped %s dupes", saved, skipped)
